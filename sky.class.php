...
class Sky {
    ...
    function getAuthorizeURL( $url, $response_type = 'code', $state = NULL, $display = NULL ) {
        ...
        $params['appSignature'] =hash_hmac("sha1","appKey=".App_Key."&timestamp=".time()."",App_Secret);
...
class SkyHandle
{
    ...
    {
        $this->oauth = new Sky($akey,$skey,$access_token);
    }
...
//        print_r($arr);die;
     ...
     * @return xml
     */
    function getFolderInfo($access_token)
    {
        $method='GET';
        return $this->get('/getFolderInfo.action',$method,$access_token);
    }
     ...
     * @return xml
     */
    function listFiles($access_token)
    {
        $method='GET';
     ...
     * @return xml
     */
    function getUserInfoExt($access_token)
    {
        $method='GET';
    ...
    function getFileDownloadUrl($access_token,$id)
    {
        $method='GET';
        $parameters['id']=$id;
        $parameters['short']='true';
        return $this->get('/getFileDownloadUrl.action',$method,$access_token,$parameters);
    }
    ...
    function uploadFile($access_token,$file=array())
    {
        $method='GET';
    ...
    function createUploadFile($access_token,$file=array())
    {
        $method = 'GET';
        $xml = $this->get('/getFolderInfo.action', $method, $access_token);
    ...
    function get($url,$method,$access_token,$parameters = array()) {
        ...
        $sign=hash_hmac('sha1',"AccessToken=$access_token&Operate=$method&RequestURI=$url&Date=$time",App_Secret);
        ...
        if($parameters['id']) {
            ...
            $url = "http://api.cloud.189.cn" . $url . "?"."fileId=".$parameters['id']."&short=$parameters[short]";

        } elseif($parameters['pic']=="iconOption"){

            $url = "http://api.cloud.189.cn" . $url . "?"."$parameters[pic]=1";
    ...
    function post($url,$method,$access_token,$parameters = array()){
        ...
        if(isset($parameters['id'])&&!empty($parameters['id'])) {
            ...
            $sign=hash_hmac('sha1',"AccessToken=$access_token&Operate=$method&RequestURI=$parameters[commit_url]&Date=$time",App_Secret);
    ...
    function put($url,$method,$access_token,$parameters = array()){
        ...
        if(isset($parameters['file_status'])&&$parameters['file_status']==0) {
            ...
            $file_sign = hash_hmac('sha1', "AccessToken=$access_token&Operate=$method&RequestURI=$parameters[upload_url]&Date=$time",App_Secret);
            ...
            $file_sign = hash_hmac('sha1', "AccessToken=$access_token&Operate=$method&RequestURI=$url&Date=$time",App_Secret);
